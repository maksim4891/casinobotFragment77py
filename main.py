...
# --- СИСТЕМА ПОПОЛНЕНИЯ (УМНЫЙ ПЕРЕБОР АДРЕСОВ С ЛОГИРОВАНИЕМ) ---
def create_crypto_invoice(amount, user_id):
    # Каскадный список адресов: если один лежит или заблокирован, пробуем следующий
    urls = [
        "https://pay.cryptobot.site/api/createInvoice",
        "https://pay.cryptobot.in/api/createInvoice",
        "https://pay.cryptobotsite.com/api/createInvoice",
        "https://pay.crypto-bot.site/api/createInvoice"
    ]
    payload = {
        "asset": "USDT", 
        "amount": str(amount), 
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=7) as response:
                ...
                if res_json.get("result"):
                    print(f"[CryptoBot Success]: Счет успешно создан через {url}")
                    return res_json["result"]["pay_url"], res_json["result"]["invoice_id"]
        except urllib.error.HTTPError as e:
            print(f"[CryptoBot HTTP Error] Домен: {url} | Код: {e.code} | Ответ: {e.read().decode()}")
        except Exception as e:
            print(f"[CryptoBot System Error] Домен: {url} | Ошибка: {e}")
...
def check_crypto_invoice(invoice_id):
    urls = [
        ...
        "https://pay.cryptobotsite.com/api/getInvoices",
        "https://pay.crypto-bot.site/api/getInvoices"
    ]
...
def process_deposit_amount(message):
    try:
        ...
        if not pay_url:
            bot.send_message(message.chat.id, "❌ Ошибка генерации счета. Запрос заблокирован сервером платежки. Проверь вкладку «Логи работы» (Logs) на хостинге.")
            return
        markup = types.InlineKeyboardMarkup()
