...
ADMIN_ID = 1178979444
CRYPTO_BOT_TOKEN = '586791:AATP4ja0WXJqRzxPtfWJUdGEyCUWeS73EoB'
...
def init_db():
    ...
    # Проверка на случай, если старая таблица vouchers имела старую структуру (добавляем/меняем столбцы)
    try:
...
# --- СИСТЕМА ПОПОЛНЕНИЯ (ЗАЩИЩЕННАЯ ОТ БЛОКИРОВОК С USER-AGENT) ---
def create_crypto_invoice(amount, user_id):
    ...
    for url in urls:
        try:
            ...
            # Маскируемся под реальный браузер, чтобы обойти Cloudflare
            ...
            print(f"[CryptoBot Error {url}]: Код {e.code}, причина: {e.read().decode()}")
        except Exception as e:
            print(f"[CryptoBot Error {url}]: {e}")
    return None, None

def check_crypto_invoice(invoice_id):
    urls = [
        "https://pay.cryptobot.in/api/getInvoices",
        "https://pay.cryptobot.site/api/getInvoices"
    ]
    ...
    for url in urls:
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as response:
                res_json = json.loads(response.read().decode())
                if res_json.get("result") and res_json["result"]["items"]:
...
def admin_cmd(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🎫 Создать ваучер (% к депу)", callback_data="adm_create_voucher"),
        types.InlineKeyboardButton("📢 Сделать рассылку", callback_data="adm_broadcast")
    )
    bot.send_message(message.chat.id, "🛠 <b>Панель администратора V9.0</b>", parse_mode="HTML", reply_markup=markup)
...
def callback_listener(call):
    ...
    if call.data == "main_menu":
        ...
        # ==========================================
        # 🚀 РАКЕТКА (Сетка по твоим шансам из ТЗ)
        # ==========================================
    ...
    # АДМИНКА
        ...
        msg = bot.send_message(chat_id, "Введите данные ваучера:\n`[КОД] [ПРОЦЕНТ Бонуса] [АКТИВАЦИИ]`\n\nПример: <code>START50 50 100</code>", parse_mode="HTML")
...
def process_deposit_amount(message):
    try:
        ...
        if not pay_url:
            bot.send_message(message.chat.id, "❌ Ошибка генерации счета. Запрос заблокирован сервером платежки. Проверь логи (Logs) хостинга.")
            return
        markup = types.InlineKeyboardMarkup()
        ...
        bot.send_message(message.chat.id, f"Счет на {amount}$ создан успешно:", reply_markup=markup)
    except Exception: bot.send_message(message.chat.id, "Ошибка ввода. Введите целое число.")
...
def check_payment(call):
    ...
    if check_crypto_invoice(invoice_id):
        # Проверяем, есть ли у пользователя активный процентный ваучер
        ...
        if percent > 0:
            ...
            # Удаляем использованный ваучер у игрока
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
...
def process_activate_voucher(message):
    ...
    if not voucher:
        bot.send_message(message.chat.id, "❌ Такого ваучера не существует или его срок действия истек.")
        conn.close()
        return
        
    percent, uses_left = voucher[0], voucher[1]
    
    # Проверяем, не активировал ли его этот юзер ранее
    cursor.execute("SELECT 1 FROM voucher_history WHERE user_id = ? AND code = ?", (user_id, code))
    already_used = cursor.fetchone()
    
    if already_used:
        bot.send_message(message.chat.id, "❌ Вы уже активировали этот ваучер ранее!")
        conn.close()
        return
        
    if uses_left <= 0:
        bot.send_message(message.chat.id, "❌ Активации для этого ваучера закончились.")
        conn.close()
        return
        
    # Все проверки пройдены: закрепляем ваучер за игроком
    ...
    bot.send_message(
        message.chat.id, 
        f"🎫 <b>Ваучер успешно активирован!</b>\n\nПри следующем пополнении счета вы получите <b>+{percent}%</b> к сумме депозита бесплатно!", 
        parse_mode="HTML"
    )
...
def process_admin_voucher(message):
    try:
        c, p, u = message.text.split()
        p, u = int(p), int(u)
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("INSERT OR REPLACE INTO vouchers (code, percent, uses_left) VALUES (?,?,?)", (c, p, u))
        conn.commit()
        conn.close()
        bot.send_message(message.chat.id, f"✅ Процентный ваучер <b>{c}</b> на <b>+{p}%</b> бонуса успешно создан!", parse_mode="HTML")
    except Exception as e: 
...
if __name__ == '__main__':
    init_db()
    print("Казино V9.0 успешно запущено!")
